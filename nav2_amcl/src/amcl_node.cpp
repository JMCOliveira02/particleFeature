...
#include "nav2_amcl/FeatureStruct.hpp"
#include "nav2_amcl/MapLoader.hpp"
...
{
  RCLCPP_INFO(get_logger(), "Configuring with features");
...
AmclNode::laserReceived(sensor_msgs::msg::LaserScan::ConstSharedPtr laser_scan)
{
  ...
  // Has the particle filter been initialized with a pose? Yes, if pf_init == false
  if (!pf_init_) {
...
void
AmclNode::initFeatureMap()
{
  RCLCPP_INFO(get_logger(), "initFeatureMap");
  map_loader_.loadToGlobalMap("/home/joao/ros2_ws/src/robot_worlds/feature_maps/iilab.yaml");
